 ...
 *  3. Saves a PriceSnapshot per coin to MongoDB
 *  4. For each tracker:
...
public class PriceCheckScheduler {
    ...
    public void checkPrices() {
        ...
        for (Tracker tracker : allTrackers) {
            ...
            try {
                ...
                log.error("Tracker {} has invalid threshold '{}'. Skipping.", tracker.getId(), tracker.getThreshold());
                continue;
            }
    ...
    private void handleBreach(Tracker tracker, double currentPrice, double threshold) {
        String message = String.format(
                ...
                tracker.getId(), tracker.getCoinId(), currentPrice, threshold, tracker.getUserId()
        );

        // 1. Log to console
        log.warn(message);
        System.out.println("[BREACH] " + message);

        // 2. Save Alert document
        Alert alert = new Alert();
        alert.setUserId(tracker.getUserId());
        alert.setTrackerId(tracker.getId());
        alert.setCoinId(tracker.getCoinId());
        alert.setThreshold(tracker.getThreshold());
